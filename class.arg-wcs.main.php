<?php
defined('ARG_WCS_VERSION') or exit('No direct script access allowed');
...
final class ARG_CWT
{
    ...
    public static function init()
    {
        if (!self::$initiated) {
            ...
            load_plugin_textdomain('ry-wc-city-select', false, plugin_basename(dirname(ARG_WCS_PLUGIN_BASENAME)) . '/languages');
            ...
            if (self::$use_geonames_org) {
                if (!is_dir(ARG_WCS_PLUGIN_DIR . self::$geonames_org_path)) {
                    if (!function_exists('unzip_file')) {
                        require_once ABSPATH . 'wp-admin/includes/file.php';
                        WP_Filesystem();
                    }

                    $result = unzip_file(ARG_WCS_PLUGIN_DIR . self::$geonames_org_path . '.zip', ARG_WCS_PLUGIN_DIR);
                    if ($result !== true) {
    ...
    public static function load_scripts()
    {
        if (defined('WC_VERSION')) {
            if (is_cart() || is_checkout() || is_wc_endpoint_url('edit-address')) {
                wp_enqueue_script('ry-wc-city-select', ARG_WCS_PLUGIN_URL . 'style/js/city-select.js', ['jquery', 'woocommerce'], ARG_WCS_VERSION, true);
